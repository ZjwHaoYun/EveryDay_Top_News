...
<style scoped>
/* 页面整体样式 - 关键：移除max-width，让容器和Header同宽（Header是100%自适应） */
.hot-rank-page {
  margin: 24px auto;
  padding: 0 20px;
  width: 100%;
...
/* 卡片容器 - 核心修改：grid列数改为4列 + 移除max-width保证和Header对齐 + 微调间距适配4列 */
.card-container {
  display: grid;
  grid-template-columns: repeat(4, minmax(240px, 1fr)); /* 每行4列，最小宽度240px保证适配 */
  gap: 16px; /* 微调间距，4列下16px更紧凑协调 */
  margin: 0 auto; /* 居中，无max-width则和父容器（Header）完全同宽 */
}

/* 无数据提示 - 适配4列：grid-column跨所有4列 */
.empty-tip {
  grid-column: 1 / -1;
  text-align: center;
  padding: 60px 0;
...
.hot-list {
  flex: 1; /* 列表占满剩余空间 */
  max-height: 260px; /* 4列下略减高度，保证卡片整体协调 */
  overflow-y: auto;
  overflow-x: hidden;
...
/* 响应式适配 - 核心调整：按屏幕宽度逐步降级列数，保证移动端体验 */
/* 大屏（≥1200px）：保持4列；中屏（992-1199px）：3列；小屏（600-991px）：2列；超小屏（<600px）：1列 */
